...
CREATE TABLE questoes (
  ...
  -- Novo tipo e alternativas totalmente nullables no banco, seguradas pelo CHECK
  ...
  -- Constraints vitais de consistência baseada em tipo_questao
  CHECK (
    (tipo_questao = 'MULTIPLA_ESCOLHA' AND alternativa_a IS NOT NULL AND alternativa_b IS NOT NULL AND alternativa_c IS NOT NULL AND alternativa_d IS NOT NULL AND correta IN ('A','B','C','D','E'))
    OR 
...
INSERT INTO questoes (id, disciplina, topico_id, edital_ref, banca, ano, dificuldade, enunciado, alternativa_a, alternativa_b, alternativa_c, alternativa_d, correta, justificativa, fonte_legislacao, prioridade, revisado_por_humano, criado_em, tipo_questao)
SELECT id, disciplina, topico_id, edital_ref, banca, ano, dificuldade, enunciado, alternativa_a, alternativa_b, alternativa_c, alternativa_d, correta, justificativa, fonte_legislacao, prioridade, revisado_por_humano, criado_em, 'MULTIPLA_ESCOLHA'
FROM questoes_temp;
