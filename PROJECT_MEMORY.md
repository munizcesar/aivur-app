...
- **Raio-X Editorial no Hero (coluna direita 40%)** — ✅ CONCLUÍDO. Substituído placeholder "Aivo — em breve" pela "Radiografia do Edital": bloco de texto bruto do CTB em `rgba(107,153,179,0.38)` com fade-mask vertical, trecho extraído marcado inline (`background: #FBEBD0 / color: #0A2E45`, `border-radius: 0`), linha conectora vertical vermelha (`2px, elite-red`), bloco de questão gerada com `border-left: 3px elite-red` e tag "Gerado: Questão VUNESP". Container com `rotate(-1deg)` (removido no mobile). Sem opacity herdada: cor com alpha direto no elemento pai. Commit `f1e1bda`.
- **Padronização Rota `/questoes` (Wizard / Caderno de Questões)** — ✅ CONCLUÍDO. Aplicada identidade "Curadoria Institucional Elite". Background `var(--elite-navy)`. Removidos botões gradiente, border-radius pill (9999px) e glassmorphism pesado. Cards e painéis de filtros ajustados para fundo translúcido sutil (`rgba(251,235,208,0.03)`) com borda fina `rgba(107,153,179,0.18)`. Botões primários (`qfResolverBtn`, `qfExitContinueBtn`) brutalistas retangulares (`border-radius: 2px`) em `elite-red` com shadow-offset wine (`4px 4px 0 #6B0000`). Tipografia pesada mantida (elite-cream). Atualizados `Wizard.module.css`, `WizardStep3.tsx` e `questoes/page.tsx`. Commit `e194179`.
- **Desobstrução de UX e Light Mode** — ✅ CONCLUÍDO. Restaurado toggle claro/escuro via `[data-theme="light"]` com mapeamento semântico invertido da Paleta Elite (mantendo legibilidade sem quebrar as classes hardcoded). Removido filtro inútil de "Alternativas (4/5)". Corrigido bloqueio fatal de base de dados via bypass `handleForceMock`, e UI de erro redesenhada no padrão brutalista de alertas para `WizardStep3`. Commit `1aa408f`.
...
### ?? DOUTRINA AIVUR
- **Iconografia Premium (Proibi��o Absoluta de Emojis):** � terminantemente proibido o uso de emojis (ex: ??, ??, ???) como elementos visuais na interface. Utilize SEMPRE a biblioteca `lucide-react` ou �cones SVG customizados. O design deve se manter fiel ao padr�o Elite e maduro.
