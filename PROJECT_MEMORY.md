...
- **Estado global do Cockpit com Zustand** - ✅ CONCLUÍDO. Criado `src/store/useStudyStore.ts` com `activeModuleId`, `activeTab`, `progressData` e mutações atômicas `setActiveModule`, `setActiveTab` e `registerAnswer`. `StudyCockpit.tsx` passou a consumir seletores individuais do store; o toggle efêmero da sidebar continua local. Validação real confirmou troca de módulo, troca de aba e contador global `1/3 respondidas`, sem alterações visuais ou laranja.
- **Integração do motor mock de questões no Cockpit** - ✅ CONCLUÍDO. `StudyCockpit.tsx` agora renderiza uma lista local de questões na aba `Questões`, com seleção e resposta via `useState`, Grid `40px minmax(0, 1fr)`, alto contraste e sem backend. A área central usa `flex-1 overflow-y-auto pb-32 custom-scrollbar`, com altura estrutural travada para scroll interno real. Expurgo de laranja concluído em `StudyCockpit.tsx` e `studyPathMock.ts`, usando `var(--color-primary)`. Validação real: `clientHeight: 622`, `scrollHeight: 1884`, scrollTop funcional e resposta registrada.
- **Blindagem geométrica da sidebar do Cockpit** - ✅ CONCLUÍDO. Como `md:w-72` e `md:flex-none` não foram aplicados de forma confiável pelo JIT, `StudyCockpit.tsx` passou a forçar `width: 288px` e `flex: 0 0 288px` inline. Validação real confirmou sidebar desktop de 288px e off-canvas mobile entre `left: -288px` e `left: 0`.
- **Rota isolada do Cockpit** - ✅ CONCLUÍDO. Criada `src/app/sala-de-aula/page.tsx`, invólucro puro que monta `StudyCockpit` em `/sala-de-aula` sem tocar no domínio `/Trilhas`, sem API e sem roteamento interno. Diagnóstico do arquivo passou; validação visual ficou pendente porque o servidor local não respondeu à navegação RSC.
- **Cockpit de Estudos estático (mock)** - ✅ CONCLUÍDO. Criados `src/mocks/studyPathMock.ts` e `src/components/Cockpit/StudyCockpit.tsx`. O componente usa somente `useState`, mock local, sidebar fixa no desktop, off-canvas no mobile e abas de Resumo Express, Flashcards e Questões. Nenhuma rota, API, `fetch` ou `useRouter` foi conectado; aguardando validação responsiva humana.
- **Blindagem final de contraste das alternativas** - ✅ CONCLUÍDO. `WizardStep3.tsx` e `FullscreenQuestion.tsx` agora forçam `text-slate-800`, `color: #1e293b`, `fontWeight: 500`, fundo branco, `rounded-xl`, sombra e `marginBottom: 12px` inline. Validação no DOM confirmou texto escuro legível sobre branco e geometria preservada.
- **Correção do espaçamento mobile das alternativas** - ✅ CONCLUÍDO. O container de opções em `FullscreenQuestion.tsx` recebia `gap: 48px` apesar da classe `gap-3`; o espaçamento foi fixado inline em `12px`. Validação no navegador confirmou `gap: 12px`, cards de `72px` e círculos de `40x40px`.
...
- **LCP Optimization (Hero Image)** - ? CONCLU�DO. Adicionado fetchPriority=\high\ na tag \<img>\ do \hero-main.png\ (Dashboard.tsx) para for�ar o carregamento imediato e eliminar o delay visual relatado pelo usu�rio, otimizando o LCP (Largest Contentful Paint). Commit: 'perf(ui): prioriza carregamento da imagem principal do hero para eliminar delay visual'.
...
- **Elimina��o de Flicker de Tema (Logo)** - ? CONCLU�DO. A l�gica JS (hydration mismatch) que alternava o logo no \Header.tsx\ com base no tema foi substitu�da por renderiza��o est�tica simult�nea com \
ext/image\ (usando \lock dark:hidden\ e \hidden dark:block\). Adicionado \priority\ para evitar delay, melhorando a m�trica CLS e eliminando o piscar da vers�o errada no FOUC. Commit: 'fix(ui): substitui logica de tema do logo js por css puro para eliminar flicker no carregamento'.
...
- **Corre��o Definitiva do Logo (Flicker/Contraste)** - ? CONCLU�DO. O Tailwind v4 neste projeto n�o estava mapeando a variante \dark:\ para \[data-theme="dark"]\ nativamente, o que quebrou as classes \dark:hidden\. A solu��o definitiva e blindada foi criar classes puras de CSS Module (\.logoLight\ e \.logoDark\) no \Header.module.css\ que escutam diretamente o seletor \:global([data-theme="dark"])\. Isso corrigiu o contraste nos dois temas e manteve o carregamento instant�neo sem flicker de JS. Commit: 'fix(ui): implementa toggle de logo via CSS Modules para contornar limitacao do Tailwind v4'.
...
- **Blindagem inline das alternativas** - ✅ CONCLUÍDO. A grade das opções agora usa `gridTemplateColumns: "40px minmax(0, 1fr)"` diretamente no DOM em `FullscreenQuestion.tsx` e `WizardStep3.tsx`. Fundo branco, texto escuro e dimensões `40x40px` do círculo também são definidos inline, eliminando dependência do JIT do Tailwind para o contraste e a geometria. Validado no navegador com colunas computadas `40px 613px`, círculo `40x40px` e texto `rgb(31,41,55)`.
