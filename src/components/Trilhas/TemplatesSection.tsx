    ...
    <section className="mt-12 pt-8 border-t border-[rgba(107,153,179,0.2)]" aria-labelledby="templates-heading">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--color-slate-blue)] mt-1 max-w-2xl">
            Inicie com editais oficiais pré-mapeados e checklists prontos para estudo imediato sem precisar de digitação.
          </p>
        </div>

        {/* CONTADOR */}
        <div className="text-xs text-[var(--color-slate-blue)] flex items-center gap-2">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-3 mb-6 scrollbar-none" role="tablist">
        <span className="text-xs text-[var(--color-slate-blue)] font-semibold flex items-center gap-1 pl-1 pr-1 flex-shrink-0">
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-150 flex-shrink-0 cursor-pointer ${
                isActive
                  ? "bg-[var(--color-red)] text-[var(--color-cream)] shadow-[2px_2px_0px_#6B0000]"
                  : "bg-[var(--color-navy)]/40 hover:bg-[var(--color-navy)] text-[var(--color-slate-blue)] hover:text-[var(--color-cream)] border border-[rgba(107,153,179,0.2)]"
              }`}
            >
              {cat}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredTemplates.map((template) => {
          return (
            <div
              key={template.id}
              className="flex flex-col justify-between rounded-xl border border-[rgba(107,153,179,0.2)] hover:border-[rgba(107,153,179,0.45)] bg-[var(--color-navy)]/25 hover:bg-[var(--color-navy)]/40 p-5 transition-all duration-200 hover:-translate-y-1 shadow-sm backdrop-blur-sm group"
            >
              <div>
                {/* TOPO: BADGE & CATEGORIA */}
                <div className="flex items-center justify-between gap-2 mb-3">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-[var(--color-slate-blue)]">
                    {template.category}
                  </span>
                  {template.badge && (
                ...
                {/* TÍTULO E BANCA */}
                <h3 className="text-lg font-bold text-white group-hover:text-[var(--color-cream)] transition-colors leading-snug">
                  {template.title}
                </h3>
                {template.banca && (
                ...
                {/* DESCRIÇÃO */}
                <p className="text-xs text-[var(--color-slate-blue)] mt-2 line-clamp-3 leading-relaxed">
                ...
                {template.syllabusSummary && template.syllabusSummary.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    ...
                    {template.syllabusSummary.length > 3 && (
                      <span className="text-[10px] px-1.5 py-0.5 text-[var(--color-slate-blue)]">
                        +{template.syllabusSummary.length - 3}
                      </span>
                    )}
                  </div>
              ...
              <div className="mt-5 pt-4 border-t border-[rgba(107,153,179,0.15)]">
                <div className="flex items-center justify-between text-xs text-[var(--color-slate-blue)] mb-4">
                  <span className="flex items-center gap-1">
                    <BookOpen className="w-3.5 h-3.5 text-[var(--color-slate-blue)]" />
                    {template.subjectsCount} matérias
                  </span>
                  <span className="flex items-center gap-1">
                    <Layers className="w-3.5 h-3.5 text-[var(--color-slate-blue)]" />
                    {template.topicsCount} tópicos
                  </span>
                  <span className="flex items-center gap-1">
                    <Clock className="w-3.5 h-3.5 text-[var(--color-slate-blue)]" />
                ...
                {template.isReady ? (
                  <Link
                    href={`/trilhas/${template.courseId}`}
                    title="Usar template (criar trilha pre-configurada)"
                    className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-[var(--color-red)] hover:bg-[#6B0000] text-[var(--color-cream)] text-xs sm:text-sm font-bold shadow-[2px_2px_0px_#6B0000] active:translate-x-[1px] active:translate-y-[1px] transition-all"
                  >
